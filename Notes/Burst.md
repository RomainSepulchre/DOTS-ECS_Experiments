...
- [Is unity.mathematics randon deterministic](https://discussions.unity.com/t/is-unity-mathematics-random-deterministic/928509/5)
- [Low-level engine development with Burst](https://www.youtube.com/watch?v=BpwvXkoFcp8)
- [SIMD at Insomniac Games: How We Do the Shuffle ](https://gdcvault.com/play/1022248/SIMD-at-Insomniac-Games-How)
...
SIMD stands for *Single Instruction, Multiple Data*, it's a way to take advantage of the fact a CPU can perform some mathematical operations on a `float4` as fast as it can be performed on a single float.

If a job still need a little bit more performance after we follow all the previous best pratices and avoided the common performance pitfall, it may be time for SIMD optimizations. To be precise SIMD optimization are useful when our job seems efficient enough in term of memory access but it perform a lot of mathematical operations on large data sets (ex: custom culling, manipulating large amounts of pixels/vertex data for procedural content, VFX, custom physics simulation).

When you think it might be the case, the first thing to do is to open the *Burst Inspector* and look at the assembly code generated for the job. It obviously helps to be able to read assembly but even if it's not the case, it's easy to count the SIMD and scalar instructions to get a sense of our job complexity:

- If we have lots of instructions with the form ***xxx*ps** (such as **addps**or **mulps**) feeding into each other, those are **vectorised SIMD instructions** so that's good.
- If we have lots of instructions with the form ***xxx*ss** (such as **addss**or **mulss**), those are **scalar instructions** and that's not good.

The goal of SIMD optimizations is to count the SIMD and scalar instructions and refactor the code to help Burst remove as many scalar insctructions as possible.

The [Intrinsics: Low-level engine development with Burst](https://www.youtube.com/watch?v=BpwvXkoFcp8) unity video is an excellent introduction to how convert some code to SIMD. Here a short list of general best pratices and advices for writing SIMD-friendly code with burst given in the video:

- **Become familiar with the Burst inspector**. Even without being an assembly expert, viewing assembly can give a good idea on how optimal our code is and we can compare the assembly produced once we start making change.
- **Eliminate branches**. A linear set of instructions is faster for CPU than branching conditionnal code.
- **Prefer wider batches of input data**. The whole concept of Data Oriented Design(DOD) is to process many things at once rather than processing things individually.
- **Use Unity.Mathematics vertically**. Don't use a `float3` to represent a x,y,z vector when we can use three `float4` (one for x values, one for y values and one for z values) and use SIMD to process the 4 vectors in the same amount of code it would have take to process one.

[SIMD at Insomniac Games: How We Do the Shuffle](https://gdcvault.com/play/1022248/SIMD-at-Insomniac-Games-How) is another interesting presentation about the topic.
